...
- ✅ **Tap to open** ([#85](https://github.com/H1shamM/ai-email-copilot/issues/85) / PR [#86](https://github.com/H1shamM/ai-email-copilot/pull/86)) — `/inbox` now attaches a `📖 #<id>` button per email (2/row); tapping → `cb_email_view` → the detail view, no typing. `/email <id>` kept as fallback. Verified live: tapping #15359 opened the matching email with its action keyboard.

#### UX deep pass via Chrome MCP (2026-06-05)
A UX-only pass over every command (triage/time-saving lens) filed 7 findings ([#87](https://github.com/H1shamM/ai-email-copilot/issues/87)–[#93](https://github.com/H1shamM/ai-email-copilot/issues/93)). Verdict: reading/understanding the inbox is fast (inbox cards + tap-open + `/agent` "what's urgent"); acting/clearing is still slow (one-at-a-time, no bulk). Shipped so far:
...
- ✅ **Personalized replies** ([#98](https://github.com/H1shamM/ai-email-copilot/issues/98)) — **Phase 1** (PR [#99](https://github.com/H1shamM/ai-email-copilot/pull/99)): learn the user's voice from Sent mail (`app/ai/voice.py`, cached in `user_preferences`), inject as few-shot into the reply prompt; verified the fetch/clean/select pipeline picks up real samples (formal EN, casual, Hebrew). **Phase 2** (PR [#100](https://github.com/H1shamM/ai-email-copilot/pull/100), closes [#91](https://github.com/H1shamM/ai-email-copilot/issues/91)): collapse 3 tones → **one voice draft + ✏ Shorter/Warmer nudges** (one Claude call = faster). Verified the flow is live; generation currently blocked by **Anthropic credit balance too low** (top up to use — not a code issue; error handling degrades gracefully to "Couldn't draft a reply").
- ✅ **Tappable agent ids** ([#93](https://github.com/H1shamM/ai-email-copilot/issues/93) / PR [#103](https://github.com/H1shamM/ai-email-copilot/pull/103)) — informational `/agent` replies attach `📖 #<id>` buttons (max 6, deduped) for every email referenced, reusing `cb_email_view`.
- ✅ **Edit-flow footgun** ([#101](https://github.com/H1shamM/ai-email-copilot/issues/101) / PR [#102](https://github.com/H1shamM/ai-email-copilot/pull/102)) — found via live MCP test: ✏ Edit then typing **sent the email instantly** with no review. Now editing updates the draft and re-shows it with the action keyboard to review + tap Send; prompt reworded.
- 🟢 **All 7 UX findings + both new bugs shipped.** Follow-ups (nice-to-have): dedupe near-identical voice samples; optional `/learn`.
- ⚠️ **Action item (user):** add Anthropic API credits — all AI features (`/reply`, `/agent`, `/analyze`, push) are blocked until then.
